...
static int	init_args(char **argv, t_data *data)
{
	...
	if (it_is_digit(argv[2]) && ft_atoi(argv[2]) != -1)
		data->time_to_die = ft_atoi(argv[2]);
	else
		exit (print_error(TTD_ARG));
	if (it_is_digit(argv[3]) && ft_atoi(argv[3]) != -1)
		data->time_to_eat = ft_atoi(argv[3]);
	else
		exit (print_error(TTE_ARG));
	if (it_is_digit(argv[4]) && ft_atoi(argv[4]) != -1)
...
static int	init_mutex(t_data *data)
{
	...
	if (pthread_mutex_init(&data->print, NULL) != 0)
		return (EXIT_FAILURE, print_error(ERR_MUTEX));
